...
export default function LiveUserList() {
  ...
  const connectedUsersMap = loginUser?.connections?.reduce(
    (acc, connection) => {
      const userX =
  ...
  const handleLiveUserData = async () => {
    try {
      ...
      enqueueSnackbar(error.message || "Unable to fetch live users data.", {
        variant: "error",
      });
    } finally {
      setIsLoading(false);
  ...
  const handleUserConnection = async (remoteUserId) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        enqueueSnackbar("Connected successfully! Redirecting to chat...", {
          variant: "success",
        });
      } else {
        enqueueSnackbar(
          response.message || "Can not connect to user, please try again.",
          { variant: "error" }
        );
      }
    } catch (error) {
      enqueueSnackbar(
        error.message || "Can not connect to user, please try again.",
        { variant: "error" }
      );
    } finally {
      setIsLoading(false);
  ...
  const filteredUsers = liveUserData?.filter((user) => {
    ...
    if (user?.username === loginUser?.username) return false;
      ...
      <div className="flex-1 bg-gradient-to-r from-black to-gray-800 h-full overflow-auto">
        <header className="h-fit sticky top-0 p-4 md:flex items-center justify-center w-full z-10 bg-gradient-to-r from-black to-gray-800 sm:space-x-2">
          <div className="flex justify-between items-center">
            ...
            <div className="flex md:hidden text-green-500 items-center space-x-1">
              <span className="text-sm">
                {liveUserData?.length > 0 ? liveUserData.length - 1 : 0}
              </span>
          ...
          >
            ...
            <MenuItem onClick={() => handleClose("Not Joined")}>
              Not Joined
            </MenuItem>
          </Menu>
        </header>
        ...
        {isLoading ? (
          ...
          <div className="p-4 mx-auto flex-1 md:w-[39rem] lg:w-[47rem]">
